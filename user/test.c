...
void testing_trace(){
    ...
    if (cpid==0){
        ...
        mask = (1<< SYS_fork)|(1<< SYS_sleep); //you can uncomment this inorder to check you print for both fork and sleep syscalls
    ...
    mask= (1<< SYS_sbrk)|( 1<< SYS_exit) | (1<< SYS_kill); //sbrk & exit & kill
    trace(mask, getpid());
    cpid= fork();
    kill(cpid);
    sbrk(4096);
